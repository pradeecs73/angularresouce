...
exports.checkPermission = function(req, res, next) {
	if (!req.user) {
		return res.status(401).send([{"permission":'Access denied'}]);
	} else if (req.user.role.permissions[req.feature][req.action]) {
		next();
	} else {
		return res.status(403).send([{"permission":'Unauthorized'}]);
	}
...
exports.companyAuthenticationRole = function(req, companyId) {
    return (req.user.company._id+'' == companyId || req.role.pristine == true);
}